...
def validatePassport(passport):
    ...
    if(allFieldsPresent):
        ...
        if(re.match(r'^\d{4}$', passportFields['byr']) is None or int(passportFields['byr']) < 1920 or int(passportFields['byr']) > 2002):
            dataValid = False
            # print('Failed byr (len!=4, outside 1920-2002)')

        # iyr (Issue Year) - four digits; at least 2010 and at most 2020.
        if(re.match(r'^\d{4}$', passportFields['iyr']) is None or int(passportFields['iyr']) < 2010 or int(passportFields['iyr']) > 2020):
            dataValid = False
            # print('Failed iyr (len!=4, outside 2010-2020)')

        # eyr (Expiration Year) - four digits; at least 2020 and at most 2030.
        if(re.match(r'^\d{4}$', passportFields['eyr']) is None or int(passportFields['eyr']) < 2020 or int(passportFields['eyr']) > 2030):
            dataValid = False
        ...
        if(re.match(r'^\d{2,3}(in|cm)$', passportFields['hgt']) is not None):
            height = int(passportFields['hgt'][0:len(passportFields['hgt']) - 2])
            if(re.match(r'^\d{2,3}cm$', passportFields['hgt']) is not None):
                if(height < 150 or height > 193):
                    dataValid = False
                    # print('Failed hgt (cm) 150-193')
            elif(re.match(r'^\d{2,3}in$', passportFields['hgt']) is not None):
        ...
        if(re.match(r'^#[0-9,a-f]{6}$', passportFields['hcl']) is None):
        ...
        if(re.match(r'^[0-9]{9}$', passportFields['pid']) is None):
            # re.match(regex, content) is not None:
            dataValid = False
    ...
    if(passportFields['valid'] is True):
